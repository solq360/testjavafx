 ...
 *
 * <a href="http://my.oschina.net/u/244147" class="referer" target="_blank">@see
 * </a> javafx.scene.media.MediaPlayer
 * <a href="http://my.oschina.net/u/244147" class="referer" target="_blank">@see
 * </a> javafx.scene.media.Media
 */
public class OverlayMediaPlayer extends Application {
    private static final String MEDIA_URL = "file:///g:/test.mp4";
	...
	public PlayerPane(final MediaPlayer mp) {
		...
		public void handle(MouseEvent t) {
		    ...
		    transition = ParallelTransitionBuilder.create()
			    .children(FadeTransitionBuilder.create().node(mediaTopBar).toValue(1).duration(Duration.millis(200)).interpolator(Interpolator.EASE_OUT).build(),
				    FadeTransitionBuilder.create().node(mediaBottomBar).toValue(1).duration(Duration.millis(200)).interpolator(Interpolator.EASE_OUT).build())
			    .build();
		    transition.play();
		}
	    });
		...
		public void handle(MouseEvent t) {
			...
			transition.stop();
		    transition = ParallelTransitionBuilder.create()
			    .children(FadeTransitionBuilder.create().node(mediaTopBar).toValue(0).duration(Duration.millis(800)).interpolator(Interpolator.EASE_OUT).build(),
				    FadeTransitionBuilder.create().node(mediaBottomBar).toValue(0).duration(Duration.millis(800)).interpolator(Interpolator.EASE_OUT).build())
			    .build();
		    transition.play();
		}
	    });
	    ...
	    // Time slider
	    timeSlider = SliderBuilder.create().id("media-slider").minWidth(240).maxWidth(Double.MAX_VALUE).build();
	    ...
	    // Play label
	    playTime = LabelBuilder.create().prefWidth(130).minWidth(50).textFill(Color.WHITE).build();
	    mediaTopBar.getChildren().add(playTime);

	    // Volume label
	    Label volumeLabel = LabelBuilder.create().text("Vol").textFill(Color.WHITE).minWidth(Control.USE_PREF_SIZE).build();
	    mediaTopBar.getChildren().add(volumeLabel);

	    // Volume slider
	    volumeSlider = SliderBuilder.create().id("media-slider").prefWidth(120).maxWidth(Region.USE_PREF_SIZE).minWidth(30).build();
	    ...
	    mediaBottomBar = HBoxBuilder.create().id("bottom").spacing(0).alignment(Pos.CENTER)
		    .children(ButtonBuilder.create().id("back-button").text("Back").onAction(backAction).build(), ButtonBuilder.create().id("stop-button").text("Stop").onAction(stopAction).build(),
			    ButtonBuilder.create().id("play-button").text("Play").onAction(playAction).build(), ButtonBuilder.create().id("pause-button").text("Pause").onAction(pauseAction).build(),
			    ButtonBuilder.create().id("forward-button").text("Forward").onAction(forwardAction).build())
		    .build();
